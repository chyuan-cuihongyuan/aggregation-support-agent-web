...
/** 功能特性列表 */
const FEATURES = [
  { icon: Sparkles, title: "智能对话", description: "支持多智能体选择，流式响应，Markdown 渲染" },
  { icon: Cpu, title: "工具调用", description: "强大的工具调用能力，执行复杂任务" },
  { icon: Database, title: "知识库", description: "文档上传向量化，支持多种检索方式" },
  { icon: Network, title: "AIOps 分析", description: "一键告警分析，提供运维建议" },
];
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await register({ username, password, email: email || undefined, nickname: nickname || undefined });
      router.push("/login");
    } catch (err) {
        ...
        <div className="w-full max-w-md">
          <Card className="border-border/50">
            <CardHeader className="space-y-1">
              ...
              <CardDescription>创建账号以访问平台</CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleRegister} className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="username" className="text-sm font-medium">用户名 *</label>
                  <Input id="username" type="text" placeholder="4-32位字母数字下划线" value={username}
                ...
                <div className="space-y-2">
                  ...
                  <Input id="nickname" type="text" placeholder="选填" value={nickname}
                    onChange={(e) => setNickname(e.target.value)} disabled={isLoading} />
                </div>
                <div className="space-y-2">
                  <label htmlFor="email" className="text-sm font-medium">邮箱</label>
                  <Input id="email" type="email" placeholder="选填" value={email}
                    onChange={(e) => setEmail(e.target.value)} disabled={isLoading} />
                </div>
                <div className="space-y-2">
                  <label htmlFor="password" className="text-sm font-medium">密码 *</label>
                  <Input id="password" type="password" placeholder="6-64位" value={password}
